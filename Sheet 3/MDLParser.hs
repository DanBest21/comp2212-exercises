...
happyExpList = Happy_Data_Array.listArray (0,38) ([7168,49153,1,0,2048,512,8192,0,0,0,8192,0,32,64,0,0,8192,0,0,0,0,0,0,284,16384,7168,1,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parseCalc","Exp","Action","Check","Digit","Num","Cond","';'","forward","left","right","'?<'","digit","num","if","then","else","%eof"]
        bit_start = st * 20
        bit_end = (st + 1) * 20
...
happyReduction_7 (HappyTerminal (MDLDigit _ happy_var_1))
	 =  HappyAbsSyn7
		 (happy_var_1
	)
...
happyReduction_8 (HappyTerminal (MDLNumber _ happy_var_1))
	 =  HappyAbsSyn8
		 (happy_var_1
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	MDLBreak _ -> cont 10;
	MDLForward _ -> cont 11;
	MDLRotateLeft _ -> cont 12;
	MDLRotateRight _ -> cont 13;
	MDLCheck _ -> cont 14;
	MDLDigit _ happy_dollar_dollar -> cont 15;
	MDLNumber _ happy_dollar_dollar -> cont 16;
	MDLIf _ -> cont 17;
	MDLThen _ -> cont 18;
	MDLElse _ -> cont 19;
	_ -> happyError' ((tk:tks), [])
